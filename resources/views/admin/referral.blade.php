...
<div class="referral-grid">
  <section class="referral-card">
    ...
    <form method="POST" action="{{ route('admin.referral.settings.update') }}">
      ...
      <div class="referral-field"><label for="referral_expiry_days">Pending referral expiration (days)</label><input id="referral_expiry_days" name="referral_expiry_days" type="number" min="1" max="3650" step="1" value="{{ old('referral_expiry_days', $settings['referral_expiry_days'] ?? 90) }}" required><small style="display:block;margin-top:5px;color:var(--muted);font-size:11px">Pending referrals with no qualifying completed order expire automatically after this many days. Default: 90 days.</small></div>
