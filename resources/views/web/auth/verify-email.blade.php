@extends('layouts.app')
@section('title', (session('locale') === 'ar' ? 'تأكيد الإيميل' : 'Verify Your Email') . ' — Ramo Store')

@section('content')
@php $isAr = session('locale') === 'ar'; @endphp
<div class="page" dir="{{ $isAr ? 'rtl' : 'ltr' }}" style="max-width:460px;margin:0 auto;{{ $isAr ? 'text-align:right;font-family:Tahoma,Arial,sans-serif' : '' }}">
  <div class="auth-card">
    ...
    @if(session('registered'))
      <div style="background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:10px;padding:12px 16px;margin-bottom:16px;color:#15803d;font-size:13px;display:flex;align-items:center;gap:8px;text-align:left">
        ...
        {{ $isAr ? 'حسابك اتعمل! فاضل خطوة واحدة بس: أكد إيميلك.' : 'Account created! One last step — verify your email.' }}
      </div>
    @endif

    <h2 class="auth-title" style="text-align:center">{{ $isAr ? 'بص في إيميلك' : 'Check your inbox' }}</h2>
    <p class="auth-sub" style="text-align:center">
      {{ $isAr ? 'بعتنالك لينك تأكيد على' : 'We sent a verification link to' }}<br>
    ...
    @if(session('expired'))
      <div style="background:#fef3c7;border:1.5px solid #fcd34d;border-radius:10px;padding:12px 16px;margin-bottom:16px;color:#92400e;font-size:13px">
        {{ $isAr ? 'اللينك اللي فات انتهى. دوس تحت عشان تبعت واحد جديد.' : 'Your previous link has expired. Click below to get a new one.' }}
      </div>
    @endif
    ...
    @if(session('resent'))
      <div style="background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:10px;padding:12px 16px;margin-bottom:16px;color:#15803d;font-size:13px;display:flex;align-items:center;gap:8px">
        <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        {{ $isAr ? 'لينك التأكيد اتبعت تاني!' : 'Verification link resent!' }}
      </div>
    @endif

    @if(session('dev_url'))
      <div style="background:#fffbeb;border:1.5px dashed #f59e0b;border-radius:10px;padding:14px;margin-bottom:16px">
        <div style="font-size:11px;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">{{ $isAr ? 'وضع التطوير — لينك تأكيد الإيميل' : 'Dev Mode — Verification Link' }}</div>
    ...
    <div style="background:#f9fafb;border-radius:12px;padding:16px 18px;margin-bottom:20px;font-size:13px;color:#555;line-height:1.6">
      <strong style="color:#1a1a1a;display:block;margin-bottom:4px">{{ $isAr ? 'ماوصلكش؟' : "Didn't get it?" }}</strong>
      {{ $isAr ? 'بص في الـSpam أو Junk، أو ابعت اللينك تاني من تحت.' : 'Check your spam or junk folder, or resend the link below.' }}
    </div>
    ...
    <form method="POST" action="{{ route('email.verify.resend') }}">
      ...
      <button type="submit" class="btn btn-dark" style="width:100%;justify-content:center;border-radius:10px;padding:13px;font-size:14px">
        {{ $isAr ? 'ابعت لينك التأكيد تاني' : 'Resend Verification Link' }}
      </button>
    </form>
    ...
    <form method="POST" action="{{ route('logout') }}" style="margin-top:12px">
      ...
      <button type="submit" style="width:100%;background:none;border:none;color:#888;font-size:13px;cursor:pointer;padding:8px">
        {{ $isAr ? 'سجّل خروج واستخدم حساب تاني' : 'Sign out and use a different account' }}
      </button>
    </form>
  </div>
</div>
