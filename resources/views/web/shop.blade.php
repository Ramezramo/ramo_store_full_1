  ...
  <div class="shop-layout">
    ...
    <aside class="sidebar" id="shop-sidebar">
      ...
      <div class="cat-widget">
        ...
        {{-- All Products --}}
        @php $totalProducts = $products->total(); @endphp
        ...
        @foreach($parentCats as $parent)
          ...
          <div class="cat-parent-item">
            ...
            @if($hasChildren)
              {{-- Clickable row: name links to category, chevron toggles children --}}
              <div class="cat-parent-btn {{ $isActive ? 'active' : '' }} {{ $isOpen && !$isActive ? 'parent-open' : '' }}"
              ...
              @if($isActive || $isOpen)
              <style>
                #parent-row-{{ $parent->id }} { background: {{ $isActive ? 'var(--c-orange)' : 'var(--c-tag)' }}; border-radius:9px; }
              </style>
              @endif

            @else
              <a href="{{ $parentUrl }}"
                 class="cat-parent-btn {{ $isActive ? 'active' : '' }}">
                ...
                <div class="cat-children-inner">
                  @foreach($childCats[$parent->id] as $child)
                    ...
                    <a href="{{ $childUrl }}" class="cat-child-link {{ $childActive ? 'active' : '' }}">
                      ...
                      @if($childCount > 0)
                        <span class="cat-count-badge">{{ $childCount }}</span>
                      @endif
                    </a>
                  @endforeach
                </div>
