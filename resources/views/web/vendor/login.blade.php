...
<div class="box">
  ...
  @if($errors->has('email'))
    <div class="{{ str_contains($errors->first('email'), 'review') ? 'warn-box' : 'err-box' }}">
      {{ $errors->first('email') }}
    </div>
  @endif
  @if($errors->has('password'))
    <div class="err-box">{{ $errors->first('password') }}</div>
  @endif
  ...
  <form method="POST" action="{{ route('vendor.login.submit') }}">
    ...
    <div class="form-group">
      ...
      <input type="email" name="email" value="{{ old('email') }}" class="{{ $errors->has('email') ? 'err' : '' }}" required autofocus placeholder="your@email.com">
    </div>
    <div class="form-group">
      <label>Password</label>
      <input type="password" name="password" class="{{ $errors->has('password') ? 'err' : '' }}" required placeholder="••••••••">
    </div>
  ...
  <div class="links">
    <div>New seller? <a href="{{ route('vendor.register') }}">Apply now</a></div>
