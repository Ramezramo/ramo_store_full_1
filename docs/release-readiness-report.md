...
| Automated regression suite | `php artisan test` passed **108 tests and 486 assertions** after the IDOR remediation on 15 August 2026. | **Passed for covered behavior.** |
...
| Order and cart IDOR controls | Customer order-note reads and writes now require ownership authorization through `OrderPolicy`; customer reads expose customer-visible notes only; cart item updates retain an authenticated `user_id` predicate on the final mutation. | **Resolved in code and regression-tested.** |
...
|---|---|
| Full Laravel suite | **108 passed, 486 assertions** after the IDOR remediation. |
...
| IDOR ownership tests | Passed: cross-customer order-note read/write denial, customer-visible note filtering, and cart item owner scoping. |
