...
<div class="page">
  ...
  <?php $__empty_1 = true; $__currentLoopData = $sections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $si => $sec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
    ...
    <?php elseif($layout === 'productCustomizer'): ?>
      <?php
      ...
      <?php if($fp): ?>
        ...
        ?>
        <?php if($st): ?>
        <div class="sec-head" style="margin-bottom:20px"><h2 class="sec-title"><?php echo e($st); ?></h2></div>
        <?php endif; ?>
        <div class="pcw-card" style="margin-bottom:44px">
          <a href="<?php echo e(route('product', $fp->id)); ?>" class="pcw-img-wrap">
            <?php if($fp->thumbnail_url): ?>
              <img src="<?php echo e($fp->thumbnail_url); ?>" alt="<?php echo e($fp->name); ?>" class="pcw-img" loading="lazy">
            <?php else: ?>
              <div class="pcw-img-placeholder">🛍️</div>
            <?php endif; ?>
            <?php if($pcwHasDisc): ?><span class="badge-sale"><?php echo e(round($pcwDiscPct)); ?>% OFF</span><?php endif; ?>
          </a>
          <div class="pcw-info">
            <div class="pcw-title-row">
              <h2 class="pcw-title"><a href="<?php echo e(route('product', $fp->id)); ?>" style="color:inherit;text-decoration:none"><?php echo e($fp->name); ?></a></h2>
              <?php if($showWishlist): ?>
              <button class="pcw-wish-btn <?php echo e($pcwInWishlist ? 'wished' : ''); ?>"
                      onclick="toggleWishlist(this, <?php echo e($fp->id); ?>)"
                      title="<?php echo e($pcwInWishlist ? 'Remove from Wishlist' : 'Add to Wishlist'); ?>"><?php echo e($pcwInWishlist ? '♥' : '♡'); ?></button>
              <?php endif; ?>
            </div>
            <?php if($showRating): ?>
            <div class="pcw-rating-row">
              <div class="pcw-stars">
                <?php for($s=1;$s<=5;$s++): ?><span style="color:<?php echo e($s<=round($pcwAvg)?'#f5a623':'#ddd'); ?>">★</span><?php endfor; ?>
              </div>
              <?php if($pcwTotal): ?>
                <span class="pcw-rating-val"><?php echo e($pcwAvg); ?></span>
                <a href="<?php echo e(route('product', $fp->id)); ?>#reviews" class="pcw-rating-cnt">(<?php echo e($pcwTotal); ?> review<?php echo e($pcwTotal!=1?'s':''); ?>)</a>
              <?php else: ?>
            ...
            <div class="pcw-stock">
              <?php if($fp->stock_quantity > 0): ?>
                <span class="pcw-stock-ok">✓ In Stock (<?php echo e(number_format($fp->stock_quantity)); ?> available)</span>
              <?php else: ?>
            ...
            <div class="pcw-price-block">
              <div class="pcw-price-row">
                <?php if($pcwIsRange): ?>
                  <span class="pcw-price on-sale"><?php echo e(number_format((float)$pcwMinEff,2)); ?> – <?php echo e(number_format((float)$pcwMaxEff,2)); ?> EGP</span>
                <?php elseif($pcwHasDisc): ?>
                  <span class="pcw-price on-sale"><?php echo e(number_format((float)$pcwMinEff,2)); ?> EGP</span>
                  <span class="pcw-price-orig"><?php echo e(number_format((float)$pcwMinReg,2)); ?> EGP</span>
                <?php else: ?>
                  <span class="pcw-price"><?php echo e(number_format((float)$pcwMinEff,2)); ?> EGP</span>
                <?php endif; ?>
                <?php if($pcwHasDisc): ?><span class="pcw-disc-badge"><?php echo e(round($pcwDiscPct)); ?>% OFF</span><?php endif; ?>
              </div>
              <?php if($pcwHasDisc): ?>
              <div class="pcw-sale-note">🏷️ Sale price — you save <?php echo e(round($pcwDiscPct)); ?>% off the original price</div>
              <?php endif; ?>
            </div>
            <?php if($showVariations && !empty($pcwAttrMap)): ?>
            <div class="pcw-variations">
              <?php $__currentLoopData = $pcwAttrMap; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attrKey => $attrValues): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php $isColor = strtolower($attrKey) === 'color'; ?>
                <div class="pcw-var-group">
                  <div class="pcw-var-label"><?php echo e(strtoupper($attrKey)); ?></div>
                  <div class="pcw-var-opts">
                    <?php $__currentLoopData = $attrValues; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                      <?php if($isColor): ?>
                        <span class="pcw-swatch" title="<?php echo e($val); ?>" style="background:var(--swatch-<?php echo e(Str::slug($val)); ?>,#999)"></span>
                      <?php else: ?>
                        <span class="pcw-chip"><?php echo e($val); ?></span>
                      <?php endif; ?>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                  </div>
                </div>
            ...
            <div class="pcw-cart-row">
              <div class="qty-input">
                ...
                <input type="number" value="1" min="1" max="<?php echo e($fp->stock_quantity ?: 99); ?>" id="pcw-qty-<?php echo e($si); ?>">
                <button type="button" onclick="this.previousElementSibling.value=Math.min(<?php echo e($fp->stock_quantity ?: 99); ?>,+this.previousElementSibling.value+1)">+</button>
              </div>
              <button class="pcw-atc-btn"
                      onclick="addToCart(<?php echo e($fp->id); ?>, '<?php echo e(addslashes($fp->name)); ?>', <?php echo e((float)$pcwMinEff); ?>, '<?php echo e($fp->thumbnail_url); ?>', parseInt(document.getElementById('pcw-qty-<?php echo e($si); ?>').value)||1)">
                🛒 Add to Cart
              </button>
            </div>
            <?php if($showCoupon): ?>
            <div class="pcw-coupon-wrap">
              ...
              <div class="pcw-coupon-row">
                <input type="text" id="pcw-coupon-<?php echo e($si); ?>" class="pcw-coupon-input" placeholder="Enter promo code" maxlength="50">
                <button class="pcw-coupon-btn" onclick="applyPcwCoupon('pcw-coupon-<?php echo e($si); ?>','pcw-msg-<?php echo e($si); ?>')">Apply</button>
              </div>
              <div id="pcw-msg-<?php echo e($si); ?>" class="pcw-coupon-msg"></div>
            </div>
            <?php endif; ?>
            <a href="<?php echo e(route('product', $fp->id)); ?>" class="pcw-view-link">View full details →</a>
          </div>
        </div>
      <?php endif; ?>
